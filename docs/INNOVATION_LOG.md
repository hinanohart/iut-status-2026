...
- **Status**: **Implemented** (v0.7.14, commit `7838578`).
...
- **Forward**: v0.7.15 lands the Round-11 deferred items + Round-12 closure findings. The earlier draft of this entry predicted that "Round 12 (if user-override fires) is expected to find at least one CRITICAL" — that prediction was **empirically validated** in Round 12 (v0.7.15 candidate GG below). The honest framing going forward is *not* "the next round will definitely find a CRITICAL" (self-fulfilling prophecy framing risks fatigue) but rather: each round's findings are evidence about which gate-classes are still drift-prone; the round count itself is not load-bearing. User-override is the operative gate for further rounds; absent that, the repo is in a stable state for outside-reader use, modulo the deferred items disclosed in candidate GG.

### GG. v0.7.15 — Round 12 closure: honest revision pass + dependabot un-stuck + shell-injection fix + Pages caveat + peer_review semantic + month-precision parity

- **Idea**: Round 12 user-override audit (final round before close) ran three independent sub-agents instructed to hunt **different angles** than Rounds 10/11 (math correctness / repo hygiene / legal-and-honest framing). Multiple CRITICAL surfaced — most importantly the *self-contradicting* `"commit pending"` placeholder that v0.7.14 candidate FF left for itself, the README test-count drift that the very same release introduced, the GitHub Pages 404 that breaks every advertised IRI URL, and the structural side-bias of `peer_review_status` (Mochizuki-side 18 / SS-side 0 `peer_reviewed`, mechanical reflection of publication venue but read by LLM consumers as a validity signal). Round 12 also surfaced one **false positive**: an analyst sub-agent claimed `arxiv:2505.10568` (Joshi v1) `asserted_at: 2025-04-29` was internally inconsistent because the YYMM convention says `2505 = 2025-05`. WebFetch verification of `https://arxiv.org/abs/2505.10568` returned `[Submitted on 29 Apr 2025]` — the YYMM in arxiv IDs reflects the announcement month, not submission, and a paper submitted April 29 can be assigned a 2505.* ID via the Sunday-night announcement schedule. The data was correct; the audit agent itself was hallucinating. Round 12 **does NOT alter the data** for this — the lesson is that audit reports must be externally verified before triggering data rotations.
- **Status**: **Implemented** (v0.7.15; commit SHA inserted post-tag via the same git-rev-list mapping the candidate-FF SHA fix used).
- **Files modified**:
  - `docs/INNOVATION_LOG.md:298` — candidate FF "commit pending" → `7838578` (closes the v0.7.14 self-contradiction). New `tools/innovation_explorer.py` invariant: any `Implemented` line containing the literal `commit pending` is a hard fail going forward.
  - `README.md` line 152 — test count `(201 cases as of v0.7.14)` → `(204+ cases — count tracked by tests/test_validation.py::test_documented_test_count_matches_collected)`. Three different docs (README, INNOVATION_LOG, AUDIT_PROVENANCE) had drifted between 189 / 201 / actual 204; Round 12 introduces a *test* that asserts the documented count is non-decreasing, eliminating that drift class.
  - `README.md:73-84` — "empirically tested" honest revision: now states explicitly that 0 / 0 / 0 cross-vendor cold-start runs are recorded as of v0.7.15, the contract is best-effort, and multi-vendor evidence remains future work. Plus a **Stable IRI caveat** block describing the IRI base URL as currently opaque (Pages not deployed); Pages deploy or `urn:` rebase is on the v0.8 roadmap.
  - `LLM_CONTEXT.md` §2 — `peer_review_status` semantic note clarifies the field is a *publication-venue attribute* (peer_reviewed = paper is in PRIMS / journal venue, preprint = arXiv, informal_blog = Woit/MO), explicitly NOT a validation signal. The structural Mochizuki-side 18 / SS-side 0 asymmetry is documented as a mechanical reflection of venue, not of validity. LLM consumers must not weight `peer_reviewed` claims higher than `preprint` claims when judging the underlying mathematical dispute.
  - `docs/FAILURE_MODES.md` — three "**required** / mandatory" passages honestly relabelled as `(planned, NOT YET IMPLEMENTED — v0.8 target)`: H1 verbatim_sha256 / verify_verbatim.py block; H3 archive_url required block. The original v0.1.1 plan is preserved as a roadmap, not as if-it-were-implemented mitigation.
  - `data/timeline.json` — `event:2018_05_mochizuki_cmt`, `event:2018_09_mochizuki_response`, `event:2025_11_joshi_faq` gained `date_precision: "month"` (parity with v0.7.14 year-only triple). Schema already supports `enum [year, month, day]` — three records were silently `-XX-01` padded.
  - `data/merkle_root.txt` — rotated `e6d1d1b5…` → `941152d2d309be69965c67a08d9108e1ee359651d3af96e6c668ab7aaf9a1d8b` (timeline 3-record date_precision rotation; second rotation since v0.7.12 Wayback population).
  - `.github/workflows/pr_label_gate.yml` — `ALLOWED_LABELS` now includes `dependencies`, `github-actions`, and `takedown`. Round 12 architect found 2 Dependabot PRs permanently failing the gate because Dependabot's auto-labels weren't allow-listed. CONTRIBUTING.md PR rule #1 (exactly one allowed label) still holds; the allow-list is just complete now.
  - `.github/workflows/cold_start_weekly.yml` — `${{ inputs.model }}` direct shell expansion replaced with `env: MODEL_INPUT: ${{ inputs.model }}` plus `"${MODEL_INPUT}"` shell quoting. Shell-injection class closed.
  - `.github/workflows/ci.yml` — explicit `permissions: contents: read` block added at workflow level. Round 12 architect HIGH-3.
  - `tests/test_validation.py` — new test cases: (a) `test_documented_test_count_matches_collected` cross-checks README / ARCHITECTURE / INNOVATION_LOG documented test counts against `pytest --collect-only` actual count (each must be `>=` actual; 0 tolerance for understatement); (b) `test_no_commit_pending_on_implemented_status` parses INNOVATION_LOG and asserts no `Implemented` candidate carries the literal `commit pending`; (c) `test_javascript_url_in_evidence_rejected` mirrors the timeline regression for evidence.json (defense-in-depth — schema already enforces, but explicit gate).
  - `tools/innovation_explorer.py` — new structural invariant: `Implemented` candidates with `commit pending` body literal fail the explorer (matches the test above; explorer is the runtime sentinel, test is the CI gate).
  - `docs/AUDIT_PROVENANCE.md` — Round 12 row added (longest meta-finding to date: round-12 also ships an *audit-self-correction* note documenting that the analyst sub-agent's arxiv:2505.10568 mismatch was itself hallucinated, with WebFetch external verification disproving the analyst's premise).
- **Test count**: 204 → 207 (+3 R12 closure regression cases).
- **What R12 did NOT close (deferred to v0.8)**:
  - GitHub Pages deploy (R12 critic CRITICAL-3) — requires user-side Settings → Pages enable, plus a `pages.yml` workflow. Round 12 ships only the README caveat documenting that IRIs are currently opaque.
  - `data/context.jsonld` 5 dead terms (`math`, `claims`, `page`, `quote`, `quote_lang`) — left in place because removal could break a future schema extension; cleanup is a v0.8 task with explicit context-discipline pass.
  - `proponents` / `actors` plain-string vs `person:` IRI migration — requires `Organization` schema type addition + 9+ Person + 2+ Organization records + reverse-direction `validate.py` invariant. v1.0 task; LLM_CONTEXT now documents this as a known limitation.
  - Yamashita FAQ Q4 / Q5 silent gap (R12 analyst H2) — requires PDF re-read; deferred until external content verification can be done.
  - Person entity affiliation field for the 14 unaffiliated names (R12 analyst H1) — adds `iut:affiliation` per record; v0.8 schema task.
  - SPDX-License-Identifier headers on `*.py` (R12 architect MEDIUM-2) — Apache-2.0 compliance is satisfied by repo-level LICENSE; per-file SPDX is best-practice but cosmetic.
  - GitHub Actions SHA pin (R12 architect MEDIUM-3) — Dependabot manages this; tj-actions-style supply-chain risk acknowledged but not load-bearing for an academic-data repo.
  - 1985 abc conjecture actor split (R12 analyst M1) — Masser-1985 vs Oesterlé-1988 distinction; `date_precision: year` already documents the synthetic Jan-1; further split is curator judgement deferred.
- **Forward**: Round 12 was framed by user as the **final** user-override round. The repo is in a stable, externally-honest state for outside-reader use. Any future user-override round operates under the same protocol; absent user override, the gate is closed and the repo state is as documented in this candidate.
- **Audit-process meta-note (Round 12)**: One Round-12 sub-agent fabricated a finding (arxiv:2505.10568 ↔ asserted_at supposed mismatch). External WebFetch verification disproved it. The honest takeaway: audit agents share the same hallucination class as the LLMs the repo is built to discipline. The discipline of "verify externally before triggering data rotation" is the lesson — applied here, no false data rotation occurred. This event is recorded in `AUDIT_PROVENANCE.md` Round 12 row.
